...
logging.basicConfig(
                    ...
                    handlers=[logging.FileHandler(APP_DIR+'logs/main-app|'+ str(time.asctime())+'.log'),
...
if len(config.read(CONFIGURATIONS))==0:
   logging.error("Could not find the file:"+CONFIGURATIONS)
   sys.exit()
...
if __name__ == '__main__':
 logging.debug('Dronepyapp has started!Directory %s',APP_DIR)
   
logging.info("Drone id: %s Host ip:%s", str(DRONE_ID), str(HOST_IP))
